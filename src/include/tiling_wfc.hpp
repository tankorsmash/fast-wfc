...
template <typename T> struct Tile
{
     ...
     */
    static std::vector<Array2D<T>> generate_oriented(Array2D<T> data, Symmetry symmetry);
...
template <typename T> class TilingWFC
{
     ...
     */
    static std::vector<double> get_tiles_weights(const std::vector<Tile<T>>& tiles);
    ...
    TilingWFC(
        ...
        : tiles(tiles), id_to_oriented_tile(generate_oriented_tile_ids(tiles).first),
        oriented_tile_ids(generate_oriented_tile_ids(tiles).second), options(options),
...
template <typename T>
std::vector<unsigned> Tile<T>::generate_rotation_map(const Symmetry& symmetry)
{
    switch (symmetry) {
        case Symmetry::X:
            return {0};
...
template <typename T>
std::vector<unsigned> Tile<T>::generate_reflection_map(const Symmetry& symmetry)
{
    switch (symmetry) {
        case Symmetry::X:
            return {0};
...
template <typename T>
std::vector<std::vector<unsigned>> Tile<T>::generate_action_map(const Symmetry& symmetry)
{
...
template <typename T>
std::vector<Array2D<T>> Tile<T>::generate_oriented(Array2D<T> data, Symmetry symmetry)
{
...
template <typename T>
std::pair<std::vector<std::pair<unsigned, unsigned>>, std::vector<std::vector<unsigned>>> TilingWFC<T>::generate_oriented_tile_ids(const std::vector<Tile<T>>& tiles)
{
    std::vector<std::pair<unsigned, unsigned>> id_to_oriented_tile;
    std::vector<std::vector<unsigned>> oriented_tile_ids;
...
template <typename T>
std::vector<std::array<std::vector<unsigned>, 4>> TilingWFC<T>::generate_propagator(
    const std::vector<std::tuple<unsigned, unsigned, unsigned, unsigned>>& neighbors,
    std::vector<Tile<T>> tiles, std::vector<std::pair<unsigned, unsigned>> id_to_oriented_tile,
    std::vector<std::vector<unsigned>> oriented_tile_ids )
{
...
template <typename T>
std::vector<double> TilingWFC<T>::get_tiles_weights(const std::vector<Tile<T>>& tiles)
{
    std::vector<double> frequencies;
    for (size_t i = 0; i < tiles.size(); ++i) {
        auto i_tile = tiles[i];
        for (size_t j = 0; j < i_tile.data.size(); ++j) {
            frequencies.push_back(i_tile.weight / i_tile.data.size());
        }
    }
    return frequencies;
}
